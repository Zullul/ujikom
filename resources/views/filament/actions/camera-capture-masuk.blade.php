<div x-data="{
    ...
    capturePhoto() {
        ...
        // Update hidden input menggunakan ID
        setTimeout(() => {
            const hiddenFoto = document.getElementById('foto_masuk_data_input');
            if (hiddenFoto) {
                hiddenFoto.value = this.capturedImage;
                hiddenFoto.dispatchEvent(new Event('input', { bubbles: true }));
                console.log('📸 Foto saved to hidden input');
            } else {
                console.error('Hidden input foto_masuk_data_input not found');
            }
        }, 100);
    ...
    getLocation() {
        if (!navigator.geolocation) {
            this.errorMessage = 'Browser tidak mendukung Geolocation';
            return;
        }
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                // Update hidden inputs menggunakan ID
                setTimeout(() => {
                    const hiddenLat = document.getElementById('latitude_input');
                    const hiddenLng = document.getElementById('longitude_input');
                    ...
                    if (hiddenLat && hiddenLng) {
                        ...
                        console.error('Hidden inputs for location not found');
                    }
                }, 100);
            },
            (error) => {
